...
export const InvestigatorDashboard: React.FC = () => {
  ...
  const stages = [
    { title: 'High-Resolution Document Rasterization', desc: 'Rendering PDF/image pages at 300 DPI into 8x8 block spatial grids' },
    { title: 'DCT Frequency-Domain Extraction', desc: 'Computing 2D discrete cosine transforms & sampling mid-frequency coefficients' },
    { title: 'Reed-Solomon ECC Reconstruction', desc: 'Correcting bit-errors and reconstructing 256-bit payload with parity check' },
  ...
  const handleRunAnalysis = async () => {
    ...
    const res = await ApiClient.analyzeLeakedDocument(selectedFile || undefined);
    setResult(res);
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
          ...
          <div className="glass-panel" style={{ padding: '24px' }}>
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '16px' }}>
              ...
              >
                <FolderUp size={12} color="var(--crimson-primary)" />
                <span>{selectedFile ? 'Change File' : 'Upload Any File'}</span>
              </button>
              <input
                type="file"
                ref={fileInputRef}
            ...
            <div 
              ...
              style={{
                ...
                padding: '24px 16px',
                textAlign: 'center',
                backgroundColor: selectedFile ? 'rgba(239, 68, 68, 0.08)' : 'rgba(239, 68, 68, 0.04)',
                marginBottom: '16px',
                cursor: 'pointer',
            ...
            >
              ...
              {selectedFile ? (
                <>
                  <div style={{ fontWeight: 600, color: '#ffffff', fontSize: '14px', wordBreak: 'break-all' }}>
                    {selectedFile.name}
                  </div>
                  <div style={{ fontSize: '12px', color: 'var(--cyan-primary)', marginTop: '4px' }}>
                    {(selectedFile.size / 1024).toFixed(1)} KB &bull; Custom Ingested File
                  </div>
                  <div style={{ fontSize: '11px', color: 'var(--text-muted)', marginTop: '6px' }}>
                    Click to browse another file or run attribution below
                  </div>
                </>
              ) : (
                <>
                  <div style={{ fontWeight: 600, color: '#ffffff', fontSize: '14px' }}>
                    Click to Upload Any Leaked / Damaged File
                  </div>
                  <div style={{ fontSize: '12px', color: 'var(--text-muted)', marginTop: '4px' }}>
                    Supports: PDF, Screen Photos (JPG/PNG), Cropped Documents
                  </div>
                  <div style={{ fontSize: '11px', color: 'var(--text-dim)', marginTop: '8px' }}>
                    (Or click Start Attribution to analyze pre-staged sample leak)
                  </div>
                </>
              )}
            </div>
            ...
            {selectedFile && (
              <div style={{ marginBottom: '14px', textAlign: 'right' }}>
                ...
                >
                  Reset to Pre-Staged Suspect Leak
                </button>
              </div>
            )}
            ...
            >
              {isAnalyzing ? (
                ...
                <>
                  <Search size={18} />
                  <span>{selectedFile ? `ANALYZE ${selectedFile.name.toUpperCase()}` : 'START FORENSIC ATTRIBUTION'}</span>
                </>
              )}
            </button>
          </div>
