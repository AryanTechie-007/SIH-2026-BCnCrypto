...
export const RecipientDashboard: React.FC = () => {
  ...
  const [customEncryptedFile, setCustomEncryptedFile] = useState<File | null>(null);
  ...
  const handleDecrypt = async () => {
    ...
    try {
      ...
      setErrorMessage(err.message || 'ACCESS DENIED: You are not authorized to decrypt this document.');
      setDecryptionResult(null);
      setActiveStep(0);
    } finally {
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
          ...
          <div className="glass-panel" style={{ padding: '20px' }}>
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '12px' }}>
              ...
              >
                <FolderUp size={12} color="var(--cyan-primary)" />
                <span>{customEncryptedFile ? 'Change File' : 'Upload .enc / .pdf'}</span>
              </button>
              <input 
                type="file" 
                ref={fileInputRef} 
                onChange={(e) => {
                  ...
                  if (file) setCustomEncryptedFile(file);
                }}
                accept=".enc,.pdf,application/pdf"
                style={{ display: 'none' }}
              />
            </div>

            {customEncryptedFile ? (
              <div style={{ backgroundColor: 'rgba(6, 182, 212, 0.1)', padding: '10px 12px', borderRadius: '6px', border: '1px solid rgba(6, 182, 212, 0.3)', fontSize: '12px' }}>
                <div style={{ color: 'var(--cyan-primary)', fontWeight: 600, marginBottom: '2px' }}>CUSTOM ENCRYPTED PAYLOAD:</div>
                <div style={{ color: '#ffffff', wordBreak: 'break-all' }}>{customEncryptedFile.name} ({(customEncryptedFile.size/1024).toFixed(1)} KB)</div>
                <button 
                  onClick={() => setCustomEncryptedFile(null)} 
                  style={{ background: 'none', border: 'none', color: '#f87171', fontSize: '11px', cursor: 'pointer', padding: 0, marginTop: '6px' }}
                >
                  Reset to Staged Document
                </button>
              </div>
            ) : (
              <select
          ...
          {errorMessage && (
            <div style={{
              ...
              <ShieldAlert size={20} color="var(--crimson-primary)" style={{ flexShrink: 0, marginTop: '2px' }} />
              <div>
                <strong style={{ color: '#ffffff', display: 'block', marginBottom: '2px' }}>
                  POST-QUANTUM ACCESS REJECTED
                </strong>
                {errorMessage}
              </div>
            </div>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
          ...
          {decryptionResult && (
            <div className="glass-panel glass-panel-emerald" style={{ padding: '24px' }}>
              ...
              {/* Action: Download Decrypted Watermarked PDF */}
              <div style={{ marginTop: '16px', marginBottom: '16px' }}>
                <button
                  className="btn btn-emerald"
                  onClick={() => ApiClient.downloadDecryptedFile(
                    decryptionResult.event.eventId, 
                    customEncryptedFile ? `DECRYPTED_${customEncryptedFile.name.replace('.enc', '')}` : `DECRYPTED_${selectedDoc?.fileName || 'DOCUMENT.pdf'}`
                  )}
                  style={{
                    width: '100%',
                    padding: '14px',
                ...
                >
                  <Download size={18} />
                  <span>DOWNLOAD DECRYPTED WATERMARKED PDF (WITH INVISIBLE 2D DCT STAMP)</span>
                </button>
              </div>
