...
start "CIPHERTRACE Backend" /D "%~dp0backend" cmd /k "python -m uvicorn app.main:app --host 0.0.0.0 --port 8000 --reload"

:: --------------------------------------------------------
:: 5. START FRONTEND
:: --------------------------------------------------------
echo [5/6] Starting Frontend (Port 5173)...
start "CIPHERTRACE Frontend" /D "%~dp0frontend" cmd /k "npm run dev"
